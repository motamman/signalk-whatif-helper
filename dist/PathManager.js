...
class PathManager {
    ...
    async getAllPaths(filter) {
        ...
        try {
            ...
            if (this.app.getPath) {
                vesselData = this.app.getPath('vessels.self');
     ...
     */
    async fetchPathsFromAPI(paths) {
        try {
            // Get server config
            let hostname = 'localhost';
            let port = 3000;
            let protocol = 'http';
            const configSettings = this.app.config?.settings;
            if (configSettings) {
     ...
     */
    extractPaths(obj, currentPath, paths) {
        ...
        for (const key of Object.keys(obj)) {
            if (skipProps.includes(key))
                continue;
            if (key === 'value')
                continue; // Don't recurse into value
            const child = obj[key];
     ...
     */
    async setValue(path, value, source) {
        ...
        this.app.handleMessage(PLUGIN_ID, delta);
     ...
     */
    async createPath(path, value, meta) {
